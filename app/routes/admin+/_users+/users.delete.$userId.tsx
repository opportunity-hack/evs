...
export const loader = async ({ request, params }: DataFunctionArgs) => {
	...
	const user = await prisma.user.findFirst({ where: { id: params.userId, orgId } })
	if (!user) {
...
export const deleteUserFormSchema = z.object({
	username: z
		...
		.min(1, {
			message:
				'You must enter the username of this user to delete it from the database.',
		}),
})
...
export async function action({ request, params }: DataFunctionArgs) {
	...
	const submission = await parse(formData, {
		async: true,
		schema: deleteUserFormSchema.superRefine(async ({ username }, ctx) => {
			...
			>
				<DialogHeader>
					...
					<DialogDescription>
						...
						{data.user?.name ?? data.user?.username} from the database? This
						operation is permanent, and will affect all associated events and
