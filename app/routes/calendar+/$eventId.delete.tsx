...
export const deleteEventFormSchema = z.object({
  title: z.string().min(1, {message: 'You must enter the title of this event to delete it from the database.'})
})
...
export async function action({ request, params }: DataFunctionArgs) {
  ...
  const submission = await parse(formData, {
    async: true,
    schema: deleteEventFormSchema.superRefine(
      async({ title }, ctx) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <DialogDescription>
          Are you sure you want to permanently remove {data.event?.title} from the database?.
        </DialogDescription>
